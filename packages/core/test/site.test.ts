...
const config: any = {
  ...
  brand: { name: 'AstroWind', description: 'A free template.' },
  metadata: { ogImage: '/og.png', twitterHandle: '@x' },
  i18n: { translations: { es: { brand: { description: 'Una plantilla gratuita.' } } } },
};
...
test('localize: only the declared fields are overridden', () => {
  ...
  assert.equal(es.brand.description, 'Una plantilla gratuita.');
  assert.equal(es.brand.name, 'AstroWind');       // inherited
  assert.equal(es.site, 'https://example.com');   // untouched
  assert.deepEqual(es.metadata, config.metadata); // untouched block
});

test('localize: overriding metadata leaves site alone', () => {
  const c: any = { brand: { name: 'X' }, metadata: { ogImage: '/a.png' }, i18n: { translations: { es: { metadata: { ogImage: '/b.png' } } } } };
  const es = localizeSiteConfig(c, 'es');
  assert.equal(es.metadata.ogImage, '/b.png');
  assert.equal(es.brand.name, 'X');
});

test('localize: the original config is never mutated', () => {
  localizeSiteConfig(config, 'es');
  assert.equal(config.brand.description, 'A free template.');
});
...
test('i18n: nothing to do when only Astro declares it', () => {
  ...
  assert.equal(resolveI18n({ defaultLocale: 'en', locales: ['en'] }, { translations: {} }), null);
});
...
test('i18n: overrides alongside an Astro declaration are not a conflict', () => {
  ...
  const out = resolveI18n(
    ...
    { translations: { es: { brand: { description: 'Una plantilla.' } } } },
  );
...
test("i18n: Astro's object locale form round-trips", () => {
  const out = resolveI18n(undefined, { locales: [{ path: 'espanol', codes: ['es'] }] });
  assert.equal(out?.defaultLocale, 'es');
...
test('i18n: declaring the languages on both sides is an error', () => {
  assert.throws(
    ...
    (err: Error) => {
      assert.match(err.message, /declared twice/);
      assert.match(err.message, /astro\.config/);
      assert.match(err.message, /translations/);   // and says where overrides live
      return true;
    },
  );
});

test('i18n: a translation for a locale Astro does not list is warned about', () => {
  const warnings: string[] = [];
  const original = console.warn;
  console.warn = (...args: unknown[]) => warnings.push(args.join(' '));
  try {
    resolveI18n({ defaultLocale: 'en', locales: ['en', 'es'] }, { translations: { fr: {} } });
  } finally {
