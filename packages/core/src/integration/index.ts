...
function processRobotsTxt(outDir: URL, siteUrl: string, allowAICrawlers: boolean, hasSitemap: boolean) {
  ...
  const generated = buildAutoGeneratedRobots(siteUrl, allowAICrawlers, hasSitemap);
...
/**
 * The block after the marker in robots.txt. The Sitemap line is emitted only
 * when `@astrojs/sitemap` is configured: announcing /sitemap-index.xml to
 * search engines when nothing writes it is a 404 with a promise attached.
 */
export function buildAutoGeneratedRobots(
  siteUrl: string,
  allowAICrawlers: boolean,
  hasSitemap: boolean,
): string {
